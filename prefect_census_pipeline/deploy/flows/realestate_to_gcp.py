...
@task
def upload_kaggle_key():
    kaggle_dir = os.path.expanduser("~/.kaggle")
    os.makedirs(kaggle_dir, exist_ok=True)

    kaggle_key = Secret.load("kaggle-key").get()
    kaggle_json = json.loads(kaggle_key)

    kaggle_filepath = f"{kaggle_dir}/kaggle.json"
    with open(kaggle_filepath, "w") as f:
        json.dump(kaggle_json, f)
    
    # Changing permissions of the file
    os.chmod(kaggle_filepath, 0o600)
...
def extract_realestate_data() -> str:
    ...
    with zipfile.ZipFile("usa-real-estate-dataset.zip", 'r') as zip_ref:
        zip_ref.extractall("/opt/prefect/flows/data/real_estate/")
...
@task(log_prints=True, tags='Load_realestate_gcp')
